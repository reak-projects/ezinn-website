...
const mobileScenes = [
  {
    ...
    title: 'Hotel check-in, without the wait.',
    description: 'EZinn lets guests check in, verify their identity, complete payment, and receive their room key through a self-service kiosk.'
  },
  {
    scene: 'Scene 2 — The Problem',
    title: 'Arrived and no one is at the front desk?',
    description: 'Peak arrival times or late-night check-ins often result in waiting lines, staffing pressure, or an empty reception desk.'
  },
  {
    scene: 'Scene 3 — Discovery',
    title: 'Meet EZinn.',
    description: 'Self-service hotel check-in, available 24/7 exactly when your guests need it most.'
  },
  {
    scene: 'Scene 4 — Approach',
    title: 'Welcome to EZinn',
    description: 'As the guest walks up to the kiosk, the intuitive touchscreen activates instantly to begin check-in.'
  },
  {
    scene: 'Scene 5 — Check-In Process',
    title: 'Find Your Booking',
    description: 'Guests enter their surname or booking reference and continue with identity verification and payment.'
  },
  {
    scene: 'Scene 6 — Completion',
    title: 'Checked in. Without the wait.',
    description: 'The room keycard is issued in under 60 seconds, and the guest heads straight to their room.'
  }
];
      ...
      {isMobile ? (
        <div className="story-mobile-stack">
          ...
          {mobileScenes.map((item, index) => (
            <div key={item.scene} className="story-mobile-card">
              <span className="story-card-tag">{item.scene}</span>
              <h3>{item.title}</h3>
              <p>{item.description}</p>
              <div className="story-mobile-step-indicator">
                {index + 1}/6
              </div>
            </div>
          ))}
